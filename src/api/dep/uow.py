...
class UOW(IUOW):
    ...
    async def __aenter__(self, testdb=None):
        ...
        self.user_t_rep = UserTypeRepository(
            UserTypeModel(), testdb=testdb if testdb else None
        )
        self.history_rep = HistoryRepository(
            HistoryModel(), testdb=testdb if testdb else None
        )
